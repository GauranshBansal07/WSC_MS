...
def run_pit_universe(universe_name, csv_paths, is_weekly=False, regime_method='learned_hmm'):
    ...
    port, counts, = simulate_portfolio(res_df, regimes, daily_prices)
    stats = performance_stats(port, periods_per_year)

    freq_label_short = "wk" if is_weekly else "mo"
    print_stats(stats, f"{universe_name} {freq_label} — LONG ONLY", counts, freq_label=freq_label_short)


def run_static_universe(universe_name, ticker_selection, is_weekly=False, regime_method='learned_hmm'):
...
def main():
    parser = argparse.ArgumentParser(description="Run CatBoost Momentum Strategy")
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Regime detection method (default: learned_hmm)"
    )
    ...
    elif args.index == 'nifty50':
        run_50 = True
    elif args.index == 'nifty100':
        run_100 = True
    elif args.index == 'nifty250':
        run_250 = True
